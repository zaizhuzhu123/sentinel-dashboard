 ...
 */
@Component
public class InMemFlowRuleStore extends InDatabaseRuleRepositoryAdapter<FlowRuleEntity> {
    ...
    public void publishMachineRules(String app, String ip, int port) {
        List<FlowRuleEntity> allRules = findAllByMachine(MachineInfo.of(app, ip, port));
        sentinelApiClient.setFlowRuleOfMachine(app, ip, port, allRules);
    }
